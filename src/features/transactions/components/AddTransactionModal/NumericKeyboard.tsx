...
function NumericKeyboardComponent({ onKeyPress, onDelete }: NumericKeyboardProps) {
    ...
    return (
        <View style={tw`w-full gap-2 px-1`}>
            {keys.map((row, rowIndex) => (
                <View key={`row-${rowIndex}`} style={tw`flex-row gap-2 justify-between`}>
                    {row.map((key) => {
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    ...
                                    isDelete ? onDelete() : onKeyPress(key);
                                }}
                                activeOpacity={0.6}
                                style={tw`flex-1 h-13 bg-card/60 border border-white/5 rounded-2xl items-center justify-center shadow-sm`}
                            >
                                {isDelete ? (
